...
### scer transcriptome
### weinberg ribosome distributions
### delta5_bias, delta3_bias
### green_n3bias, green_p5bias
### rtBias
...
weinberg_cts_by_codon <- file.path("~/iXnos/expts", "weinberg_20cds20", "process/cts_by_codon.size.27.31.txt")
weinberg_pi <- readRawProfiles(weinberg_cts_by_codon)
weinberg_pi <- weinberg_pi[match(names(scer), names(weinberg_pi))] # make sure transcripts in same order as transcriptome
weinberg_pi <- lapply(weinberg_pi, ## convert counts 0 to 1e-6
                      function(x) {
                        x[x==0] <- 1e-6
                        return(x)
                      })
weinberg_rho <- sapply(weinberg_pi, sum)
weinberg_rho <- weinberg_rho / sum(weinberg_rho)
weinberg_pi <- lapply(weinberg_pi,
                      function(x) {
                        x/sum(x)
                      })
...
for(i in 1:nParts) {
  ...
  assign(partName,
         value=simFootprints(scer, nRibosomes=partSize,
                             rhos=weinberg_rho, pis=weinberg_pi,
...
# 2. simulate ribosome distributions
lareau_cts_by_codon <- file.path("~/iXnos/expts", "lareau_20cds20", "process/cts_by_codon.size.27.31.txt")
lareau_pi <- readRawProfiles(lareau_cts_by_codon)
lareau_pi <- lareau_pi[match(names(scer), names(lareau_pi))] # make sure transcripts in same order as transcriptome
lareau_pi <- lapply(lareau_pi, ## convert counts 0 to 1e-6
                      function(x) {
                        x[x==0] <- 1e-6
                        return(x)
                      })
lareau_rho <- sapply(lareau_pi, sum)
lareau_rho <- lareau_rho / sum(lareau_rho)
lareau_pi <- lapply(lareau_pi,
                      function(x) {
                        x/sum(x)
                      })
...
# 2. simulate ribosome distributions
green_cts_by_codon <- file.path("~/iXnos/expts", "green_20cds20", "process/cts_by_codon.size.27.31.txt")
green_pi <- readRawProfiles(green_cts_by_codon)
green_pi <- green_pi[match(names(scer), names(green_pi))] # make sure transcripts in same order as transcriptome
green_pi <- lapply(green_pi, ## convert counts 0 to 1e-6
                      function(x) {
                        x[x==0] <- 1e-6
                        return(x)
                      })
green_rho <- sapply(green_pi, sum)
green_rho <- green_rho / sum(green_rho)
green_pi <- lapply(green_pi,
                      function(x) {
                        x/sum(x)
                      })
...
# ### delta5_bias, delta3_bias
# ### use green_p5bias for both f5 and f3 bias
# ### rtBias
# 
# set.seed(68)
# 
...
# 
# simulation_name <- "scer_biasDelta_p5bias_extraBase"
# if(!(simulation_name %in% list.files(outputDir))) {
#   system(paste("mkdir", file.path(outputDir, simulation_name)))
# }
# 
...
#                              delta5=delta5_bias, delta3=delta3_bias,
#                              ligBias=green_p5bias, RTBias=rtBias, circBias=green_p5bias,
#                              digest_transcript=digest_transcript))
#   writeFootprintsFA(get(partName),
#                     file.path(outputDir, simulation_name, paste0(part_filename, ".fa")))
#   save(list=partName,
