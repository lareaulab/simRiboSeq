...
scer_rho <- simRho(scer_YAL,
                   exptLengths=weinberg_lengths, exptAbundances=weinberg_abundances)
scer_pi <- simPi(scer_YAL, codonTE=weinberg_codonTE, pad5=83, pad3=83)
...
footprints <- simFootprints(scer_YAL, nRibosomes=partSize,
...
writeFootprintsFQ(footprints, file.path(output_dir, "pauseCGA.fq"))
...
write.table(fp_stats, file="pauseCGA_footprint_counts.txt",
            quote=F, row.names=F, col.names=T, sep="\t")

# save and quit -----------------------------------------------------------

save(scer_rho, scer_pi, file=file.path(output_dir, "pauseCGA_rho_pi.Rda"))
save(footprints, file=file.path(output_dir, "pauseCGA_footprints.Rda"))
