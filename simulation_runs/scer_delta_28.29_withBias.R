...
scriptDir <- file.path(here(), "scripts")
outputDir <- file.path(here(), "outputs")
refDir <- file.path(here(), "refData")
...
# digest()
delta5_28_29 <- c(0.9, 0.1, 0)
names(delta5_28_29) <- as.character(15:17)
delta3_28_29 <- c(0, 1, 0)
names(delta3_28_29) <- as.character(9:11)
...
# yeast transcriptome
# delta5, delta3: 90% 28mer frame 0, 10% 29mer frame 2
# A site: weinberg codon-level TE
# p5bias, n3bias: bias from green data
...
# delta5_28_29, delta3_28_29
# minSize=27, maxSize=31
# green data for ligBias (3') and circBias (5')

simulation_name <- "scer_delta_28.29_withBias"
if(!(simulation_name %in% list.files(outputDir))) {
  dir.create(file.path(outputDir, simulation_name))
}
...
for(i in 1:nParts) {
  ...
  assign(partName,
         value=simFootprints(scer, nRibosomes=partSize,
                             ...
                             delta5=delta5_28_29, delta3=delta3_28_29,
