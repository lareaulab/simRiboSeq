...
writeTranscriptomeFA(transcripts_seq, 
                     file.path(outputDir, "riboViz_sampleData_transcripts_18cds18.fa"))
...
save(transcripts_rho, transcripts_pi,
     file=file.path(outputDir, "riboViz_sampleData_rho_pi.Rda"))
...
exptName <- "riboViz_sampleData_footprints"

if(!(exptName %in% list.files(outputDir))) {
  system(paste("mkdir", file.path(outputDir, exptName)))
}

for(i in 1:nParts) {
  print(paste("Part", i, "of", nParts))
  partName <- paste0(exptName, "_part_", i)
  assign(partName, 
...
write.table(footprint_cts, file=file.path(outputDir, "riboViz_sampleData_rpfCounts.txt"),
