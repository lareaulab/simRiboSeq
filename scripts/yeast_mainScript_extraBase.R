...
yeast_uniform <- readFAfile(file.path(outputDir, "yeast_uniformCodons.fa"), 
                            pad5=20, pad3=20)
...
for(i in 1:nParts) {
  ...
  partName <- paste0("yeast_uniform_uniform_noBias_extraBase_part", i)
  part_filename <- paste0("yeast_uniformCodons_uniformDelta_noBias_extraBase_80Mreads_part", i)
  assign(partName, 
         value=simFootprints(yeast_uniform, nRibosomes=partSize, 
                             ...
                             ligBias=n3bias, RTBias=rtBias, circBias=p5bias,
...
for(i in 1:nParts) {
  print(paste("Part", i, "of", nParts))
  partName <- paste0("yeast_uniform_uniform_withBias_extraBase_part", i)
  part_filename <- paste0("yeast_uniformCodons_uniformDelta_withBias_extraBase_80Mreads_part", i)
  assign(partName, 
         value=simFootprints(yeast_uniform, nRibosomes=partSize, 
                             rhos=yeast_uniform_rho, pis=yeast_uniform_pi,
                             delta5=delta5_uniform, delta3=delta3_uniform,
...
for(i in 1:nParts) {
  print(paste("Part", i, "of", nParts))
  partName <- paste0("yeast_yeast_uniform_noBias_extraBase_part", i)
  part_filename <- paste0("yeast_yeastCodons_uniformDelta_noBias_extraBase_80Mreads_part", i)
  assign(partName, 
         value=simFootprints(yeast_yeast, nRibosomes=partSize, 
                             rhos=yeast_yeast_rho, pis=yeast_yeast_pi,
                             delta5=delta5_uniform, delta3=delta3_uniform,
                             ligBias=n3bias, RTBiase=rtBias, circBias=p5bias,
                             digest_transcript=digest_transcript))
  writeFootprintsFA(get(partName),
                    file.path(outputDir, paste0(part_filename, ".fa")))
  save(list=partName, 
...
for(i in 1:nParts) {
  print(paste("Part", i, "of", nParts))
  partName <- paste0("yeast_yeast_uniform_withBias_extraBase_part", i)
  part_filename <- paste0("yeast_yeastCodons_uniformDelta_withBias_extraBase_80Mreads_part", i)
  assign(partName, 
         value=simFootprints(yeast_yeast, nRibosomes=partSize, 
                             rhos=yeast_yeast_rho, pis=yeast_yeast_pi,
                             delta5=delta5_uniform, delta3=delta3_uniform,
